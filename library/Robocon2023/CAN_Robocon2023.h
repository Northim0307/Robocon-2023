...
void Setpoint_Adjust_Yaw (double yaw, int motor_num, int motor_direction, double setpoint_m[4] , int optimum_speed_m, const int speed_chg_per_yaw ) 
{
	if(setpoint_m[motor_num] != 0)
	{
	if(motor_direction == CLOCKWISE)
      {
        setpoint_m[motor_num] = setpoint_m[motor_num] - yaw * speed_chg_per_yaw ;
	  }
	  else
	  {
		 setpoint_m[motor_num] = setpoint_m[motor_num] + yaw * speed_chg_per_yaw ;
...
void calc_robot_dir ( int prev_motor_direction[4], bool starting_motor[4], float *magnitude , float *angle, int motor_direction[4], double setpoint_m[4] , const int optimum_speed_m[4])
{
	float ratio_speed_m[4];
	float ratio;
...
if(*magnitude <30 )
{
	...
	else if(*angle>30 && *angle <=60)
	{
		// *robot_dir_now = UPWARD_RIGHT;
		ratio = *angle / 90;
		setpoint_m[0] = -1.5*( ratio * ratio_speed_m[0] + (1-ratio) * ratio_speed_m[0]);
		setpoint_m[1] = 0;
		setpoint_m[2] = - setpoint_m[0];
		setpoint_m[3] = 0;
	}
	...
	else if(*angle>120 && *angle <=150)
	{
		...
		ratio = (*angle-90) / 90;
		setpoint_m[0] = 0;
		setpoint_m[1] =  1.5*( ratio * ratio_speed_m[0] + (1-ratio) * ratio_speed_m[0]);
		setpoint_m[2] = 0;
		setpoint_m[3] = - setpoint_m[1];
	}
	...
	for(int k = 0; k < 4;k++)
	{
		...
		if((prev_motor_direction[k] != motor_direction[k]) && (setpoint_m[k]!= 0))
		{
			starting_motor[k] = true;
		}
		prev_motor_direction[k] = motor_direction[k];
	}
